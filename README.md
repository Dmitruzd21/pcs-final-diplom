...
| `server.page.PageEntry`   | Класс, описывающий один элемент результата одного поиска. Он состоит из имени пдф-файла, номера страницы и количества раз, которое встретилось это слово на ней        |
| `server.SearchEngine`   | Интерфейс, описывающий поисковый движок. Всё что должен уметь делать поисковый движок, это на запрос из слова отвечать списком элементов результата ответа        |
| `server.BooleanSearchEngine`   | Реализация поискового движка, которую вам предстоит написать. Слово `Boolean` пришло из теории информационного поиска, тк наш движок будет искать в тексте ровно то слово, которое было указано, без использования синонимов и прочих приёмов нечёткого поиска        |
...
Также, списки ответов для каждого слова должны быть отсортированы в порядке уменьшения поля `count`. Для этого предлагается классу `server.page.PageEntry` сразу реализовывать интерфейс `Comparable`.
...
```
[server.page.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=12, count=6}, server.page.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=4, count=3}, server.page.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=5, count=3}, server.page.PageEntry{pdf=1. DevOps_MLops.pdf, page=5, count=2}, server.page.PageEntry{pdf=Что такое блокчейн.pdf, page=1, count=2}, server.page.PageEntry{pdf=Что такое блокчейн.pdf, page=3, count=2}, server.page.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=2, count=1}, server.page.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=11, count=1}, server.page.PageEntry{pdf=1. DevOps_MLops.pdf, page=3, count=1}, server.page.PageEntry{pdf=1. DevOps_MLops.pdf, page=4, count=1}, server.page.PageEntry{pdf=Что такое блокчейн.pdf, page=2, count=1}, server.page.PageEntry{pdf=Что такое блокчейн.pdf, page=4, count=1}, server.page.PageEntry{pdf=Что такое блокчейн.pdf, page=5, count=1}, server.page.PageEntry{pdf=Что такое блокчейн.pdf, page=7, count=1}, server.page.PageEntry{pdf=Что такое блокчейн.pdf, page=9, count=1}, server.page.PageEntry{pdf=Продвижение игр.pdf, page=7, count=1}, server.page.PageEntry{pdf=Как управлять рисками IT-проекта.pdf, page=2, count=1}]
```
...
Требуется всё также возвращать в качестве результата поиска список из `server.page.PageEntry`, только `count` в нём должен теперь содержать суммарное количество раз, которое встретилось любое из слов запроса.
